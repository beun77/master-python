...
def __main__() : # Main programm of The Light Piano system
	...
	Samples,fsampling = read_file.read_test2(1000,440,44100)
	#print(Samples)
	SAMPLES = analyze_music.split(Samples,fsampling,20)
	#print(SAMPLES)
	#SAMPLES = dict()
	#SAMPLES[1] = Samples
	for sample in SAMPLES :
		...
		#print(fft)	
		Notes = analyze_notes.which_note2(fft,ref_table)
		print('Notes',Notes)
	"""
	
	folder = os.getcwd()+"/music/"
	analyze(folder,ref_table) # First we start by analyzing any new piece of music
	...
	while not exit_flag :
		...
		if music == 'exit' :
			...
			exit_flag = coach.coach(music,'none',ref_table) # Finally we read it and launch the coach option
