...
def get_infos(path) : # Reads infos about a mp3 file
	...
	return d,fsampling,form,nb_bits,buff,nb_channels,error
...
def read_music(path) : # Reads a mp3 file at the given path and returns the list of samples as well as the sampling frequency.
	...
	d,fsampling,form,nb_bits,buff,nb_channels,error = get_infos(path)
	if error == 'channels' :
		...
		if nb_channels == 2 : # stereo
			...
			cmd = [FFMPEG_BIN,'-i',path,'-ac','1',path]
		...
		# ERRATUM: ffmpeg does NOT convert stereo to mono automatically. Maybe it can do it as a conversion option? Ideally, we would convert all files to mono 16 or mono 8 (no more stereo or signed int or planar organization)
...
def read_tlp(path) : # Reads the .tlp files to produce the light animation
	...
	for sample in range(0,len(flist)) :
		Notes[sample] = list()
		for note in flist[sample].split('|') :
			Notes[samples].append(note.split(';'))
...
def read_test2(N,f,fe) : # Returns the list of N samples of a pur sinus at a frequency of f

	A = generate_sin.generate_sinus(N,f,fe)
